...
cy1, cy2, cy3, cy4 = 280, 230, 180, 140
offset = 15

going_in = {}
counter1 = set()  # Use sets to avoid duplicate entries
going_out = {}
counter2 = set()  # Use sets to avoid duplicate entries
passengers = 0
        ...
        for obj in tracked_objects:
            ...
            if int(obj_id) in going_out:
                if cy4 < (cy + offset) and cy4 > (cy - offset):
                    ...
                    cvzone.putTextRect(frame, f'{int(obj_id)}', (x1, y1), 2, 2)
                    cv2.circle(frame, (cx, cy), 4, (255, 0, 255), -1)
                    # Only add to counter1 if not already present
                    if int(obj_id) not in counter1:
                        counter1.add(int(obj_id))

            # For objects going in
            ...
            if int(obj_id) in going_in:
                if cy1 < (cy + offset) and cy1 > (cy - offset):
                    ...
                    cvzone.putTextRect(frame, f'{int(obj_id)}', (x1, y1), 2, 2)
                    cv2.circle(frame, (cx, cy), 4, (255, 0, 255), -1)
                    # Only add to counter2 if not already present
                    if int(obj_id) not in counter2:
                        counter2.add(int(obj_id))
