...
def get_system_stats():
    ...
    # Power supply (assuming battery voltage available in /sys/class/power_supply/battery/capacity)
    try:
        with open('/sys/class/power_supply/', 'r') as f:
            power_supply = int(f.read().strip())
...
def send_gps_data(lat, lon, passengers, bus_id, current_stop, next_stop, time_to_next_stop, system_stats):
    ...
    try:
        ...
        if line.startswith('$'):
            ...
            if isinstance(msg, pynmea2.types.talker.GGA):
                ...
                send_gps_data(lat, lon, passengers, bus_id, current_stop, next_stop, time_to_next_stop, system_stats)
